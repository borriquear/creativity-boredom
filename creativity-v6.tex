...
The prediction error $\epsilon$ is lognormally distributed, $\ln \epsilon \sim N((\mu -frac{\sigma^2}{2})T, \sigma \sqrt(T)$, and the inverse of the prediction error or the prediction pleasure is also lognormally distributed, $\ln p \sim N((frac{\sigma^2}{2} - \mu)T, \sigma \sqrt(T)$ (See the Appendix for the demonstration). We use Monte Carlo simulation to sampling random outcomes of the It\^{o} process with a Monte Carlo simulation.
%yosolo montecarlo
...
If the expiration time $T$ is very far in the future, then the value of the subjective experience will be very similar to the prediction pleasure, ,$ v_{t,T -t \to \infty} =  p_{t} - k$. On the other hand, if the expiration date is near, $(T-t \sim 0)$, the subjective experience is equal to prediction pleasure minus the boredom constant,$ v_{t,t \to T} =  p_{t} - k $. 
